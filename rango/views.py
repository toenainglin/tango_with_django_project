...
def show_category(request, category_name_slug):
    ...
    try:
        ...
        # The .get() method returns one model instnace or raises an exception
...
def register(request):
    ...
    if request.method == 'POST':
        # Attemp to grab info from the raw form information
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            # until we're ready to avoid inttegrity problems.
...
def user_login(request):
    # If request if POST, try to pull out the relevant info.
    if request.method == 'POST':
        # Gather username, password provided by user from login form
        ...
        # combo is valid - User obj is returned if it is
        user = authenticate(username=username, password=password)
        
        # If we have User obj, details are correct
        # If None, no user with matching credentials found
        if user:
            # Is account active? could've been disabled
            if user.is_active:
                ...
                # An inactive account used - no logging in!
                return HttpResponse("Your Rango account is disabled.")
        else:
            # Bad login details provided. So can't login
            print(f"Invalid login details: {username}, {password}")
            return HttpResponse("Invalid login details supplied.")
    # The request is not a POST, so display login form.
    # Will probably be a HTTP GET request
    else:
